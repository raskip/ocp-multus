...
| Prereqs | `make prereqs` | Cluster subscription Reader/Contributor, DNS RG child-zone + parent-delegation write, workload RG/storage permissions | Installer operator; subscription owner if the workload RG is not pre-created; DNS owner if parent zone is centralised | Creates or uses the workload RG, creates the public child zone and private cluster DNS zone, delegates the public sub-zone, creates the installer storage account, and may create a storage data-plane role assignment. |
