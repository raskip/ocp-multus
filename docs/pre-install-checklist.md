...
| 5 | Service Principal (E1–E5 pattern) | Entra admin | days–weeks | `az ad sp create-for-rbac --name ocp-installer --years 1 --skip-assignment` then add Contributor on workload RG + Network Contributor on VNet RG. If `privatelink.blob.core.windows.net` lives in a hub/private-DNS RG, also add **Private DNS Zone Contributor** on that RG/zone so Terraform can create the storage Private Endpoint A-record and VNet link. Save JSON to `~/.azure/osServicePrincipal.json` with chmod 600. | [`azure-credentials.md`](./azure-credentials.md), [`azure-identity-options.md`](./azure-identity-options.md) |
