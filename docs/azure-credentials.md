...
| VNet / network resource group | Repo-managed mode: Network Contributor on the VNet/network RG. BYO-network mode: subnet read/join on the four core OCP subnets (plus `snet-ocp-sriov` when `ENABLE_SRIOV=true`) plus route update rights on the cluster route table. | Owner / User Access Administrator / Role Based Access Control Administrator on the relevant network scopes. The BYO-network least-privilege option is documented in [`network-prereqs.md` §4a](./network-prereqs.md#4a-least-privilege-azure-rbac-for-byo-network-mode). |
| Public DNS resource group that contains the parent zone and receives the child zone (**only when `CREATE_PUBLIC_DNS=true`**) | DNS Zone Contributor | DNS/platform team. Required only when public DNS is enabled, because the Terraform path then creates/tags the child public zone `${BASE_DOMAIN}` in this RG and writes the NS delegation into the parent zone (parent-zone-only scope is not enough). Off by default — internal installs skip this. |
